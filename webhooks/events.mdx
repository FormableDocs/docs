...
This page lists every event Formable sends to your webhook endpoint. Most integrations only need **signing** events (`document_viewed`, `document_signed`, `document_completed`). **Redlining** events are listed below for the optional negotiation workflow.
...
Signing events carry a `signing` object and reference a [signature request](/walkthroughs/embedded-signing).
...
A single signer completed and signed the document. This does not mean the signed PDF is ready yet — wait for [`document_completed`](#document_completed) before fetching the [signed envelope](/api-reference/endpoint/get-signed-envelope).

<ResponseField name="signing.recipient_signature_id" type="string">
  The ID of the recipient who signed. Matches the `recipientSignatureId` returned when you [create a signature request](/api-reference/endpoint/create-signature-request).
</ResponseField>
...
```json
{
  ...
  "signing": {
    "signature_request_id": "YfdkOH7dVtaGzl-egS6NI",
    "recipient_signature_id": "rsig_789ghi"
  }
}
```
...
All required fields are filled, the signature request is complete, and the signed PDF (with audit trail) is ready to download. If there are multiple signers, this means all signers have signed. Wait for this event before calling [Get signed envelope](/api-reference/endpoint/get-signed-envelope).
...
Redlining events carry a `redlining` object and reference a [redline request](/walkthroughs/redlining). Every redlining payload includes these base fields, plus any event-specific fields listed below.
...
switch (payload.event.event_type) {
  case "document_completed":
    await onDocumentCompleted(payload.signing.signature_request_id);
    break;

...
- `test_mode` / `testMode` keeps integration traffic out of billing.
...
| Client embed | `hellosign-embedded` + `client.open(...)` | Plain `<iframe src={signingUrl}>` |
| Completion signal | Client events + callbacks (`signature_request_all_signed`, etc.) | Prefer [`document_signed`](/webhooks/events#document_signed); no Formable JS client events |
| Webhook verification | `event_hash` | `Content-Sha256` HMAC — see [Verifying signatures](/webhooks/verifying-signatures) |
...
| `hellosign-embedded` | Plain `<iframe>` | No Formable JS SDK. |
| `test_mode: true` | `testMode: true` | Excluded from billing. |
| `signature_request_all_signed` / `downloadable` | `document_signed` | Then fetch the signed PDF. |
...
**Formable** — plain iframe (no SDK, no `client_id`, no `skipDomainVerification`):
...
Mint the URL on your backend and pass only the URL to the browser. Never call Formable with your API key from client-side code.

### 4. Detect completion and download the PDF

Prefer the [`document_signed`](/webhooks/events#document_signed) webhook over polling. You can also poll signature request `status` until it is `Completed`, or read the [event stream](/api-reference/endpoint/get-signature-request-events).
...
Webhook verification differs from Dropbox Sign's `event_hash`. Formable signs the raw body with HMAC-SHA256 and sends the digest in `Content-Sha256`. See [Verifying signatures](/webhooks/verifying-signatures) and [Webhooks overview](/webhooks/overview).
...
<AccordionGroup>
  ...
  <Accordion title="No hellosign-embedded client">
    Load `signingUrl` in an iframe. There is no Formable equivalent of client events such as `sign` or `finish`. Drive UI state from webhooks or status polling instead.
  </Accordion>

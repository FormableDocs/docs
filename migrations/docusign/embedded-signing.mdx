...
| Completion UX | Often `returnUrl` redirect with an `event` query param | Keep the iframe; drive UI from webhooks or status polling |
...
| `returnUrl` | Not required | Detect completion via webhooks or polling. |
...
Detect completion with [`document_signed`](/webhooks/events#document_signed) or by polling `status` until `Completed`. Do not rely on a DocuSign-style `returnUrl` redirect to learn that signing finished. Update your UI when your backend confirms completion.
...
<AccordionGroup>
  ...
  <Accordion title="No returnUrl redirect">
    Keep the iframe until webhook or poll says `Completed`, then update your UI. Build any "signing finished" screen in your own app.
  </Accordion>

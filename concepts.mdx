...
Every workflow records **events** so you can track progress. You can receive them two ways:

- **Push** — configure a [webhook](/webhooks/overview) to get notified in real time when events occur.
- **Pull** — poll [signature request events](/api-reference/endpoint/get-signature-request-events) and [redline request events](/api-reference/endpoint/get-redline-request-events) to reconcile state.

Use webhooks to react to state changes such as a document being signed or a redline turn ending, and use the polling endpoints to backfill anything missed.
